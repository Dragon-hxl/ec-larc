...
class LMFeatureExtractor(nn.Module):
    ...
    use_cnn=False):
        ...
        if T5 in self.lm_model_name:
            self.tokenizer = T5Tokenizer.from_pretrained("t5-small",from_tf=True)#self.lm_model_name, cache_dir=".cache/"
            self.t5_encoder_model = T5EncoderModel.from_pretrained("t5-small",from_tf=True)#self.lm_model_name, cache_dir=".cache/"
